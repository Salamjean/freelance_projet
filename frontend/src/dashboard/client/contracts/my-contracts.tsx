...
export const MyContracts: React.FC<{ userId: number | null }> = ({
  userId,
}) => {
        ...
        {[1, 2, 3, 4, 5].map((star) => (
          <button
            ...
            style={{
              ...
              lineHeight: 1,
              color:
                star <= (hovered[field] || ratings[field])
                  ? "#f59e0b"
                  : "#d1d5db",
    ...
    const fetchContracts = async () => {
      try {
        const res = await axios.get(
          `http://localhost:3000/api/client/${userId}/contracts`,
        );
        const activeContracts = res.data.filter(
          (c: Contract) => c.status !== "VALIDATED" && c.status !== "COMPLETED",
        );
      ...
      case "DELIVERED":
        return (
          <span className="status-badge status-delivered">
            <PackageCheck size={12} /> Livré — En attente de validation
          </span>
        );
      case "VALIDATED":
      case "COMPLETED":
  ...
  const filteredContracts = useMemo(() => {
    ...
    const filtered = contracts.filter((contract) => {
      ...
      const bucket = [
        ...
        getFreelancerName(contract.freelancer),
  ...
  const stats = useMemo(() => {
    return {
      ...
      inProgress: filteredContracts.filter((c) =>
        ["IN_PROGRESS", "WAITING_FOR_ADVANCE"].includes(c.status),
      ).length,
      delivered: filteredContracts.filter((c) => c.status === "DELIVERED")
        .length,
      totalAmount: filteredContracts.reduce(
  ...
  const submitValidation = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:3000/api/client/${userId}/contracts/${ratingModal.contractId}/validate`,
        payload,
      );
      setContracts((prev) =>
        prev.map((c) =>
          c.id === ratingModal.contractId ? { ...c, status: "VALIDATED" } : c,
        ),
      );
      ...
      Swal.fire({
        ...
        text: hasRating
          ? "Le freelance a été noté et la mission est terminée."
  ...
  const handleDispute = async (contractId: number) => {
    ...
    try {
      await axios.put(
        `http://localhost:3000/api/client/${userId}/contracts/${contractId}/dispute`,
        { reason },
      );
      setContracts((prev) =>
        prev.map((c) =>
          c.id === contractId ? { ...c, status: "IN_PROGRESS" } : c,
        ),
      );
      Swal.fire({
        icon: "warning",
  ...
  const handleValidateAdvance = async (contractId: number) => {
    ...
    try {
      await axios.put(
        `http://localhost:3000/api/client/${userId}/contracts/${contractId}/advance/validate`,
      );
      setContracts((prev) =>
        prev.map((c) =>
          c.id === contractId ? { ...c, advanceStatus: "PAID" } : c,
        ),
      );
  ...
  const handleCancelContract = async (contractId: number) => {
    ...
    try {
      await axios.put(
        `http://localhost:3000/api/client/${userId}/contracts/${contractId}/cancel`,
      );
  ...
  if (loading) {
    return (
      <div className="loading-container">Chargement de vos contrats...</div>
    );
  }

  return (
    <div className="my-contracts-page my-contracts-redesign">
      ...
      {ratingModal.open && (
        ...
        >
          <div className="rating-modal" onClick={(e) => e.stopPropagation()}>
            ...
            >
              Votre avis aide la communauté. Donnez une note sur 5 étoiles.
            </p>
      ...
      <section className="contracts-hero">
        ...
        <div className="contracts-kpis">
          <article className="kpi-card">
            <span className="kpi-label">Contrats visibles</span>
          ...
          <article className="kpi-card">
            <span className="kpi-label">A valider</span>
        ...
        <>
          <section className="contracts-toolbar">
            <div className="toolbar-search">
              ...
              <input
                type="text"
                placeholder="Rechercher: titre, freelance, statut..."
                value={searchTerm}
          ...
          {filteredContracts.length === 0 ? (
            <div className="empty-state compact-empty">
              <Search size={42} color="#cbd5e1" />
              <h3>Aucun resultat</h3>
              <p>Aucun contrat ne correspond a votre recherche.</p>
            </div>
          ) : (
                    ...
                    <div className="contract-details">
                      ...
                      <div className="detail-item">
                        ...
                        <span>
                          ...
                          {Number(contract.amount).toLocaleString("fr-FR")} XOF
                        </span>
                      </div>
                      <div className="detail-item">
                        <Calendar size={16} />
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (contract.applicationId) {
                          navigate(
                            `/client/projects/${contract.projectId}/applications/${contract.applicationId}/sign`,
                          );
                        } else {
